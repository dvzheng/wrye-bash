...
class _MelDistributor(MelNull):
    ...
    def getSlotsUsed(self):
        ...
        # the index where we left off in the last load_data
    ...
    def _distribute_load(self, dist_specifier, record, ins, size_, readId):
        """Internal helper method that distributes a load_data call to the
        ...
        target_loader.load_data(record, ins, signature, size_, readId)
    ...
    def _apply_mapping(self, mapped_el, record, ins, signature, size_, readId):
        ...
        that element and also distributes the load_data call to the correct
        ...
        if el_type == dict:
            ...
            # into this scope on the next load_data call.
...
class MelArray(MelBase):
    ...
    def load_data(self, record, ins, sub_type, size_, readId):
        ...
        load_entry = self._element.load_data
        if self._prelude:
            self._prelude.load_data(record, ins, sub_type, self._prelude_size,
                                    readId)
...
class PartialLoadDecider(ADecider):
    ...
    def decide_load(self, record, ins, sub_type, rec_size):
        ...
        self._loader.load_data(target, ins, sub_type, self._load_size,
                             'DECIDER.' + sub_type)
...
class MelUnion(MelBase):
    ...
    def load_data(self, record, ins, sub_type, size_, readId):
        ...
        self._get_element(decider_ret).load_data(record, ins, sub_type, size_,
                                                 readId)
