...
from .patcher_dialog import PatchDialog, PBash_gui_patchers
...
class Mod_MarkMergeable(ItemLink):
    ...
    def Execute(self):
        ...
        if bush.game.check_esl:
            ...
            message = u'== %s ' % u'Python' + _(u'Mergeability') + u'\n\n'
        if yes:
            message += u'=== ' + (
...
class Mod_ListPatchConfig(_Mod_BP_Link):
    ...
    def Execute(self):
        ...
        config = bosh.modInfos.table.getItem(self._selected_item,
                                             'bash.patch.configs', {})
        _gui_patchers = [copy.deepcopy(x) for x in PBash_gui_patchers]
